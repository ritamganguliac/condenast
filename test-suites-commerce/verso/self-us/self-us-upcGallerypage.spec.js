...
let self_Gallery_Product_Query = require("../../../test-data/verso/galleryProductGraphqlQuery");
...
function testRunner() {
  context(`Fetching the data needed for the test...`, () => {
    before(function () {
      ...
      const uri_Gallery = testData.production["upc-self-us"].uri_Gallery;
      self_Gallery_Product_Query.galleryUpcProductQuery.variables.uri = uri_Gallery;
      self_Gallery_Product_Query.galleryUpcProductQuery.variables.organizationId = orgId;
      testHelper.getGalleryUpcInfo(self_Gallery_Product_Query.galleryUpcProductQuery)
        .then((data) => {
          retailerUri = data[0];
          sellerName = data[1];
          productId = data[2];
    ...
    describe(
      `UPC Gallery Product page test - Brand: Self`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        for (let i = 0; i <= 4; i++) {
          ...
          it("UPC - Click on the Price and retailer name button and verify it redirects correctly", () => {
            cy.clearCookies();
            cy.clearLocalStorage();
            cy.visit(testData.production["upc-self-us"].galleryProdUrl).wait(2000);
            cy.scrollTo("bottom");
            cy.scrollTo(0, 3000);
            ...
            if (retailerUri[i] !== undefined) {
              let randomNumber = testHelper.getRandomNumber(0, 7);
    ...
    describe(`Unified product card changes test - Product image - Brand: Self`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        it("UPC - Click on Product name and verify it should not redirects - Gallery Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-self-us"].galleryProdUrl);
          cy.wait(2000);
          ...
          cy.url().should(
            "contains",
            testData.production["upc-self-us"].galleryProdUrl
          );
        });
        ...
        it("UPC - Click on the product image and verify it redirects correctly", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-self-us"].galleryProdUrl).wait(1000);
          ...
          cy.get(selectors.upc["gallery_UPC_image"])
            ...
            .click({ force: true });
          cy.wait(3000).then(() => {
            let splitUrl = testData.production["upc-self-us"].firstRetailerGallery.split("/");
        ...
        it("UPC - Check image aspect ratio as 3:4", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-self-us"].galleryProdUrl).wait(500);
