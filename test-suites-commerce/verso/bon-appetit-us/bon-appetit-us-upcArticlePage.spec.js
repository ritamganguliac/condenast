...
let bon_Appetit_Article_Product_Query = require("../../../test-data/verso/articleProductGraphqlQuery");
...
function testRunner() {
  context(`Fetching the data needed for the upc-bon-appetit-us test...`, () => {
    before(function () {
      cy.clearCookies();
      cy.clearLocalStorage();
      // Getting retailerUri, sellerName, productId, productName data from the graphql
      const orgId = testData.production["bon-appetit"].orgId;
      const uri_Article = testData.production["upc-bon-appetit-us"].uri_Article;
      bon_Appetit_Article_Product_Query.articleProductQuery.variables.uri = uri_Article;
      bon_Appetit_Article_Product_Query.articleProductQuery.variables.organizationId = orgId;
      testHelper
        .getArticleUpcInfo(bon_Appetit_Article_Product_Query.articleProductQuery)
        .then((data) => {
          retailerUri = data[0];
          sellerName = data[1];
          productId = data[2];
      ...
      cy.visit(testData.production["upc-bon-appetit-us"].articleProdUrl).wait(500);
    });

    // To run the below test cases for random five product data from the graphql
    describe(`UPC Product page test - Brand: Bon Appetit US`,
      {
        retries: {
          runMode: 1,
        },
    ...
    describe(`Unified product card changes test - Brand: Bon Appetit US`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        it("UPC - Click on Product name it should not redirects - Article Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-bon-appetit-us"].articleProdUrl);
          cy.wait(2000);
          ...
          cy.url().should("contains",testData.production["upc-bon-appetit-us"].articleProdUrl);
        });

        // Test case: 3(Article), When click on Product image it should redirects to first retailer url in the list.
        // Step: 1. Redirects to Staging or production article page. eg, story/best-cutting-boards
        ...
        it("UPC - Product image should redirects to first retailer url in the list - Article Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-bon-appetit-us"].articleProdUrl).wait(1000);
          ...
          cy.get(selectors.upc["gallery_UPC_image"])
            ...
            .click({ force: true });
          cy.wait(3000).then(() => {
            let splitUrl = testData.production["upc-bon-appetit-us"].firstRetailerArticle.split("/");
