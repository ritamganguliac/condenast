...
let vogue_Gallery_Product_Query = require("../../../test-data/verso/galleryProduct_Vogue_US_GraphqlQuery");
...
function testRunner() {
  context(`Fetching the data needed for the test...`, () => {
    before(function () {
      ...
      const uri_Gallery = testData[Cypress.env('environment')]["upc-vogue-us"].uri_Gallery;
      vogue_Gallery_Product_Query.galleryUpcProductQuery.variables.uri =
        uri_Gallery;
      vogue_Gallery_Product_Query.galleryUpcProductQuery.variables.organizationId =
        orgId;
      testHelper
        .getGallery_Vogue_US_UpcInfo(
          vogue_Gallery_Product_Query.galleryUpcProductQuery)
        .then((data) => {
          retailerUri = data[0];
          sellerName = data[1];
          productId = data[2];
    ...
    describe(
      ...
      () => {
        ...
        for (let i = 0; i <= 1; i++) {
          ...
          // Step: 1. Redirects to production or staging gallery page. eg, slideshow/best-lug-sole-boots
          ...
          it("UPC - Click on the Price and retailer name button and verify it redirects correctly", () => {
            cy.visit(testData[Cypress.env('environment')]["upc-vogue-us"].galleryProdUrl).wait(1500);
            cy.reload(true);
            if (cy.url() == "chrome-error://chromewebdata/") {
              cy.reload(true);
            }
            let randomNumber = testHelper.getRandomNumber(
              1,
              productId.length - 1
            );
    ...
    describe(
      `Unified product card changes test - Product image - Brand: Vogue`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        it("UPC - Click on Product name and verify it should not redirects - Gallery Page", () => {
          cy.visit(testData[Cypress.env('environment')]["upc-vogue-us"].galleryProdUrl);
          cy.wait(2000);
          let randomNumber = testHelper.getRandomNumber(
            1,
            productId.length - 1
          ...
          cy.get(
            selectors.upc["article_Product_selections"] +
              productId[randomNumber]
          ).click({ force: true });
          cy.url().should(
            "contains",
            testData[Cypress.env('environment')]["upc-vogue-us"].galleryProdUrl
          );
        });
        ...
        it("UPC - Click on the product image and verify it redirects correctly", () => {
          cy.visit(testData[Cypress.env('environment')]["upc-vogue-us"].galleryProdUrl).wait(1000);
          cy.get(selectors.upc["gallery_UPC_anchor"])
            ...
            .click({ force: true });
          cy.wait(3000).then(() => {
            let splitUrl = testData[Cypress.env('environment')]["upc-vogue-us"].firstRetailerGallery.split("/");
        ...
        it("UPC - Check image aspect ratio as 3:4", () => {
          cy.visit(testData[Cypress.env('environment')]["upc-vogue-us"].galleryProdUrl).wait(500);
