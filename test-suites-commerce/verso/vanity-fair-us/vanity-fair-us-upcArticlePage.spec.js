...
let vanity_Fair_Article_Product_Query = require("../../../test-data/verso/articleProductGraphqlQuery");
...
function testRunner() {
  context(`Fetching the data needed for the upc-vanity-fair-us test...`, () => {
    before(function () {
      cy.clearCookies();
      cy.clearLocalStorage();
      // Getting retailerUri, sellerName, productId, productName data from the graphql
      const orgId = testData.production["vanity-fair"].orgId;
      const uri_Article = testData.production["upc-vanity-fair-us"].uri_Article;
      vanity_Fair_Article_Product_Query.articleProductQuery.variables.uri = uri_Article;
      vanity_Fair_Article_Product_Query.articleProductQuery.variables.organizationId = orgId;
      testHelper
        .getArticleUpcInfo(vanity_Fair_Article_Product_Query.articleProductQuery)
        .then((data) => {
          retailerUri = data[0];
          sellerName = data[1];
          productId = data[2];
      ...
      cy.visit(testData.production["upc-vanity-fair-us"].articleProdUrl).wait(500);
    });

    // To run the below test cases for random five product data from the graphql
    describe(`UPC Product page test - Brand: Vanity Fair US`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        for (let i = 0; i <= 4; i++) {
          ...
          // Step: 1. Redirects to production article page. eg, story/best-quilts-and-coverlets-review
    ...
    describe(`Unified product card changes test - Brand: Vanity Fair US`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        it("UPC - Click on Product name it should not redirects - Article Page ", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-vanity-fair-us"].articleProdUrl);
          cy.wait(2000);
          ...
          cy.url().should("contains",testData.production["upc-vanity-fair-us"].articleProdUrl);
        });

        // Test case: 3(Article), When click on Product image it should redirects to first retailer url in the list.
        // Step: 1. Redirects to production article page. eg, story/best-cutting-boards
        ...
        it("UPC - Product image should redirects to first retailer url in the list - Article Page ", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-vanity-fair-us"].articleProdUrl).wait(1000);
          ...
          cy.get(selectors.upc["gallery_UPC_image"])
            ...
            .click({ force: true });
          cy.wait(3000).then(() => {
            let splitUrl = testData.production["upc-vanity-fair-us"].firstRetailerArticle.split("/");
