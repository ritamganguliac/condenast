...
let gq_Spain_Gallery_Product_Query = require("../../../test-data/verso/galleryProductGraphqlQuery");
...
function testRunner() {
  context(`Fetching the data needed for the test...`, () => {
    before(function () {
      ...
      const uri_Gallery = testData.production["upc-gq-spain"].uri_Gallery;
      gq_Spain_Gallery_Product_Query.galleryUpcProductQuery.variables.uri = uri_Gallery;
      gq_Spain_Gallery_Product_Query.galleryUpcProductQuery.variables.organizationId = orgId;
      testHelper.getGalleryUpcInfo(gq_Spain_Gallery_Product_Query.galleryUpcProductQuery)
        .then((data) => {
          retailerUri = data[0];
          sellerName = data[1];
          productId = data[2];
    ...
    describe(
      `UPC Gallery Product page test - Brand: GQ Spain`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        for (let i = 0; i <= 4; i++) {
          ...
          it("UPC - Click on the Price and retailer name button and verify it redirects correctly", () => {
            cy.clearCookies();
            cy.clearLocalStorage();
            cy.visit(testData.production["upc-gq-spain"].galleryProdUrl).wait(2000);
            cy.scrollTo("bottom");
            cy.scrollTo(0, 3000);
            ...
            if (retailerUri[i] !== undefined) {
              let randomNumber = testHelper.getRandomNumber(1, 15);
    ...
    describe(`Unified product card changes test - Product image - Brand: GQ Spain`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        it("UPC - Click on Product name and verify it should not redirects - Gallery Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-gq-spain"].galleryProdUrl);
          cy.wait(2000);
          ...
          cy.url().should(
            "contains",
            testData.production["upc-gq-spain"].galleryProdUrl
          );
        });
        ...
        it("UPC - Click on the product image and verify it redirects correctly", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-gq-spain"].galleryProdUrl).wait(1000);
          ...
          cy.get(selectors.upc["gallery_UPC_image"])
            ...
            .click({ force: true });
          cy.wait(3000).then(() => {
            let splitUrl = testData.production["upc-gq-spain"].firstRetailerGallery.split("/");
        ...
        it("UPC - Check image aspect ratio as 3:4", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-gq-spain"].galleryProdUrl).wait(500);
