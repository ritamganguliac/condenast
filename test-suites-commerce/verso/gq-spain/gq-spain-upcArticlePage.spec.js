...
let gq_Spain_Article_Product_Query = require("../../../test-data/verso/articleProductGraphqlQuery");
...
let productName = [];
let totalProducts = 0;
let productName_withoutUndefined = [];

function testRunner() {
  context(`Fetching the data needed for the upc-gq-spain test...`, () => {
    before(function () {
      cy.clearCookies();
      cy.clearLocalStorage();
      // Getting retailerUri, sellerName, productId, productName data from the graphql
      const orgId = testData.production["gq-spain"].orgId;
      const uri_Article = testData.production["upc-gq-spain"].uri_Article;
      gq_Spain_Article_Product_Query.articleProductQuery.variables.uri = uri_Article;
      gq_Spain_Article_Product_Query.articleProductQuery.variables.organizationId = orgId;
      testHelper
        .getArticleUpcInfo(gq_Spain_Article_Product_Query.articleProductQuery)
        .then((data) => {
          retailerUri = data[0];
          sellerName = data[1];
          productId = data[2];
      ...
      cy.visit(testData.production["upc-gq-spain"].articleProdUrl).wait(500);
    });

    // To run the below test cases for random five product data from the graphql
    describe(`UPC Product page test - Brand: GQ Spain`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        for (let i = 0; i <= 4; i++) {
          ...
          it("UPC - Click on the Price and retailer name button and verify it redirects correctly", () => {
            ...
            let randomNumber = testHelper.getRandomNumber(2, 13);
    ...
    describe(`Unified product card changes test - Brand: GQ Spain`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        it("UPC - Click on Product name and it should not redirects to any other page ", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-gq-spain"].articleProdUrl);
          cy.wait(2000);
          ...
          cy.url().should("contains",testData.production["upc-gq-spain"].articleProdUrl);
        });
        ...
        it("UPC - Click on Product image, it should redirects to first retailer url in the list ", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-gq-spain"].articleProdUrl).wait(1000);
          ...
          cy.get(selectors.upc["gallery_UPC_image"])
            ...
            .click({ force: true });
          cy.wait(3000).then(() => {
            let splitUrl = testData.production["upc-gq-spain"].firstRetailerArticle.split("/");
