...
let epicurious_Article_Product_Query = require("../../../test-data/verso/articleProductGraphqlQuery");
...
function testRunner() {
  context(`Fetching the data needed for the epicurious-us test...`, () => {
    before(function () {
      cy.clearCookies();
      cy.clearLocalStorage();
      // Getting retailerUri, sellerName, productId, productName data from the graphql
      const orgId = testData.production["epicurious"].orgId;
      const uri_Article = testData.production["upc-epicurious-us"].uri_Article;
      epicurious_Article_Product_Query.articleProductQuery.variables.uri = uri_Article;
      epicurious_Article_Product_Query.articleProductQuery.variables.organizationId = orgId;
      testHelper
        .getArticleUpcInfo(epicurious_Article_Product_Query.articleProductQuery)
        .then((data) => {
          retailerUri = data[0];
          sellerName = data[1];
          productId = data[2];
      ...
      cy.visit(testData.production["upc-epicurious-us"].articleProdUrl).wait(500);
    });

    // To run the below test cases for random five product data from the graphql
    describe(`UPC Product page test - Brand: Epicurious`,
      {
        retries: {
          runMode: 1,
        },
    ...
    describe(`Unified product card changes test - Brand: Epicurious`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        it("UPC - Click on Product name it should not redirects - Article Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-epicurious-us"].articleProdUrl);
          cy.wait(2000);
          ...
          cy.url().should("contains",testData.production["upc-epicurious-us"].articleProdUrl);
        });
        ...
        it("UPC - Product image should redirects to first retailer url in the list - Article Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-epicurious-us"].articleProdUrl).wait(1000);
          ...
          cy.get(selectors.upc["gallery_UPC_image"])
            ...
            .click({ force: true });
          cy.wait(3000).then(() => {
            let splitUrl = testData.production["upc-epicurious-us"].firstRetailerArticle.split("/");
