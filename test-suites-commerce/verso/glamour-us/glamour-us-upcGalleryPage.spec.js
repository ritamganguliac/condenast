...
let glamour_Gallery_Product_Query = require("../../../test-data/verso/galleryProductGraphqlQuery");
...
function testRunner() {
  context(`Fetching the data needed for the test...`, () => {
    before(function () {
      ...
      const orgId = testData.production["Glamour"].orgId;
      const uri_Gallery = testData.production["upc-glamour-us"].uri_Gallery;
      glamour_Gallery_Product_Query.galleryUpcProductQuery.variables.uri = uri_Gallery;
      glamour_Gallery_Product_Query.galleryUpcProductQuery.variables.organizationId = orgId;
      testHelper
        .getGalleryUpcInfo(glamour_Gallery_Product_Query.galleryUpcProductQuery)
        .then((data) => {
          retailerUri = data[0];
          sellerName = data[1];
          productId = data[2];
    ...
    describe(`UPC Gallery Product page test - Brand: Glamour`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        for (let i = 0; i <= 4; i++) {
          ...
          it("UPC - Click on the Price and retailer name button and verify it redirects correctly", () => {
            cy.clearCookies();
            cy.clearLocalStorage();
            cy.visit(testData.production["upc-glamour-us"].galleryProdUrl).wait(2000);
            cy.scrollTo("bottom");
            cy.scrollTo(0, 3000);
            ...
            if (retailerUri[i] !== undefined) {
              let randomNumber = testHelper.getRandomNumber(1, totalProducts);
    ...
    describe(
      `Unified product card changes test - Product image - Brand: Glamour`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        it("UPC - Click on Product name and verify it should not redirects - Gallery Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-glamour-us"].galleryProdUrl);
          cy.wait(2000);
          ...
          cy.url().should("contains", testData.production["upc-glamour-us"].galleryProdUrl);
        });
        ...
        it("UPC - Click on the product image and verify it redirects correctly", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-glamour-us"].galleryProdUrl).wait(1000);
          ...
          cy.wait(3000).then(() => {
            let splitUrl =
              testData.production["upc-glamour-us"].firstRetailerGallery.split("/");
        ...
        it("UPC - Check image aspect ratio as 3:4", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-glamour-us"].galleryProdUrl).wait(500);
