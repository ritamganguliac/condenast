...
let teenvogue_Article_Product_Query = require("../../../test-data/verso/articleProductGraphqlQuery");
...
function testRunner() {
  context(`Fetching the data needed for the test...`, () => {
    before(function () {
      ...
      const uri_Article = testData.production["upc-teen-vogue-us"].uri_Article;
      teenvogue_Article_Product_Query.articleProductQuery.variables.uri = uri_Article;
      teenvogue_Article_Product_Query.articleProductQuery.variables.organizationId = orgId;
      testHelper
        .getArticleUpcInfo(teenvogue_Article_Product_Query.articleProductQuery)
        .then((data) => {
          retailerUri = data[0];
          sellerName = data[1];
          productId = data[2];
      ...
      cy.visit(testData.production["upc-teen-vogue-us"].articleProdUrl).wait(500);
    });
    after(function () {
      cy.clearCookies();
      cy.clearLocalStorage();
    ...
    describe(
      `UPC Product page test - Brand: Teen Vogue`,
      {
        retries: {
          runMode: 1,
        },
    ...
    describe(`Unified product card changes test - Brand: Teen Vogue`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        it("UPC - Click on Product name it should not redirects - Article Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-teen-vogue-us"].articleProdUrl);
          cy.wait(2000);
          ...
          cy.url().should("contains",testData.production["upc-teen-vogue-us"].articleProdUrl);
        });
        ...
        it("UPC - Product image should redirects to first retailer url in the list - Article Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-teen-vogue-us"].articleProdUrl).wait(1000);
          ...
          cy.get(selectors.upc["gallery_UPC_image"])
            ...
            .click({ force: true });
          cy.wait(3000).then(() => {
            let splitUrl = testData.production["upc-teen-vogue-us"].firstRetailerArticle.split("/");
