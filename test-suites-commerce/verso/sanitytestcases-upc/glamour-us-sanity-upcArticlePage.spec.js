...
function testRunner() {
  context(`Fetching the data needed for the test...`, () => {
      ...
      () => {
        ...
        for (let i = 0; i <= 2; i++) {
          ...
          it("UPC - Click on the Price and retailer name button and verify it redirects correctly", () => {
            ...
            let trimmedSellername = sellerName[i].replace(/\s+$/,"");
    ...
    describe(`Unified product card changes test - Brand: Glamour`,
    {
      retries: {
        runMode: 1,
      },
    ...
    () => {
        ...
        it("UPC - Product image should redirects to first retailer url in the list - Article Page", () => {
          ...
          for (let i = 1; i <= 50; i++) {
            cy.wait(500);
            cy.get('[data-offer-url="' +retailerUri[i] +'"]').should('be.visible');
          }
        });
      }
    );
