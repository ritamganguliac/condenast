...
let ad_Clever_Article_Product_Query = require("../../../test-data/verso/articleProductGraphqlQuery");
...
function testRunner() {
  context(`Fetching the data needed for the ad-clever-us test...`, () => {
    before(function () {
      cy.clearCookies();
      ...
      const uri_Article = testData.production["upc-ad-clever-us"].uri_Article;
      ad_Clever_Article_Product_Query.articleProductQuery.variables.uri = uri_Article;
      ad_Clever_Article_Product_Query.articleProductQuery.variables.organizationId = orgId;
      testHelper
        .getArticleUpcInfo(ad_Clever_Article_Product_Query.articleProductQuery)
        .then((data) => {
          retailerUri = data[0];
          sellerName = data[1];
          productId = data[2];
      ...
      cy.visit(testData.production["upc-ad-clever-us"].articleProdUrl).wait(500);
    });

    // To run the below test cases for random five product data from the graphql
    describe(`UPC Product page test - Brand: AD Clever US`,
      {
        retries: {
          runMode: 1,
        },
            ...
            if (retailerUri[randomNumber] !== undefined) {
              cy.get('[data-offer-retailer="' + updatedsellerName[randomNumber] + '"]')
                ...
                .then(() => {
                  ...
                  if (cy.url() !== "chrome-error://chromewebdata/") {
                    cy.url().should("contains", validateUrl[0]);
                  }
                });
            }
          });
    ...
    describe(`Unified product card changes test - Brand: AD Clever US`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        it("UPC - Click on Product name it should not redirects - Article Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-ad-clever-us"].articleProdUrl);
          cy.wait(2000);
          ...
          cy.url().should("contains",testData.production["upc-ad-clever-us"].articleProdUrl);
        });
        ...
        it("UPC - Product image should redirects to first retailer url in the list - Article Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-ad-clever-us"].articleProdUrl).wait(1000);
          ...
          cy.get(selectors.upc["gallery_UPC_image"])
            ...
            .click({ force: true });
          cy.wait(3000).then(() => {
            let splitUrl = testData.production["upc-ad-clever-us"].firstRetailerArticle.split("/");
