...
function testRunner() {
  context(`Fetching the data needed for the test...`, () => {
    ...
    describe(`UPC Product page test - Brand: GQ`,
      {
        retries: {
          runMode: 1,
        },
        ...
        for (let i = 0; i <= 4; i++) {
          ...
          it("UPC - Click on the Price and retailer name button and verify it redirects correctly", () => {
            ...
            if (retailerUri[randomNumber] !== undefined) {
              cy.get(
                '[data-offer-retailer="' + updatedsellerName[randomNumber] + '"]'
              ).contains(updatedsellerName[randomNumber]);
    ...
    describe(`Unified product card changes test - Brand: GQ`,
      {
        retries: {
          runMode: 1,
        },
      ...
      () => {
        ...
        it("UPC - Click on Product name it should not redirects - Article Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-gq-us"].articleProdUrl);
          cy.wait(2000);
          ...
          cy.url().should(
            "contains",
            testData.production["upc-gq-us"].articleProdUrl
          );
        });
        ...
        it("UPC - Product image should redirects to first retailer url in the list - Article Page", () => {
          ...
          cy.wait(3000).then(() => {
            let splitUrl =
              testData.production["upc-gq-us"].firstRetailerArticle.split("/");
