...
function testRunner() {
  context(`Fetching the data needed for the test...`, () => {
    before(function () {
      ...
      const uri_Article = testData.production["upc-vogue-uk"].uri_Article;
      ...
      () => {
        for (let i = 0; i <= 4; i++) {
          ...
          it("UPC - Click on the Price and retailer name button and verify it redirects correctly", () => {
            cy.clearCookies();
            cy.clearLocalStorage();
            cy.visit(testData.production["upc-vogue-uk"].articleProdUrl).wait(500);
            if (cy.url() == "chrome-error://chromewebdata/") {
              cy.reload(true);
            }
            let randomNumber = testHelper.getRandomNumber(2, totalProducts - 1);
    ...
    describe(`Unified product card changes test - Brand: Vogue`,
      ...
      () => {
        ...
        it("UPC - Click on Product name it should not redirects - Article Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-vogue-uk"].articleProdUrl);
          cy.wait(2000);
          ...
          cy.url().should(
            "contains",
            testData.production["upc-vogue-uk"].articleProdUrl
          );
        });
        ...
        it("UPC - Product image should redirects to first retailer url in the list - Article Page", () => {
          cy.clearCookies();
          cy.clearLocalStorage();
          cy.visit(testData.production["upc-vogue-uk"].articleProdUrl).wait(1000);
          ...
          cy.wait(3000).then(() => {
            let splitUrl =
              testData.production["upc-vogue-uk"].firstRetailerArticle.split("/");
